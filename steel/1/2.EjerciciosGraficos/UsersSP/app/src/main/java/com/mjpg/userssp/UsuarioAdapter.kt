...
class UsuarioAdapter(private val usuarios:List<Usuario>, private val listener: MainActivity):
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Glide.with(context).load(usuario.url).centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL)
            .circleCrop().into(holder.binding.imgPhoto)
        }
    }
    //El circleCrop sirve para poner la imagen dentro de un circulo
    ...
    inner class ViewHolder(view: View):RecyclerView.ViewHolder(view){
        ...
        fun  setListener(usuario:Usuario, posicion:Int){
            binding.root.setOnClickListener {listener.onClickListener(usuario,posicion) }
        }
    }
}