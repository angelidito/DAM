...
//        val id = arguments?.getLong("id", 0)
		val id = vistaModelo.getIdentificador()

		casos(id)
		val observador = Observer<Long>{
			casos(id)
		}
		vistaModelo.identificador.observe(this.viewLifecycleOwner, observador)
		...
		return when (item.itemId) {
			...
			R.id.action_save -> {
				if (mTienda != null && validateFields(
						mBinding.tilPhotoUrl, mBinding.tilPhone,
						mBinding.tilName
					)
				) {
					with(mTienda!!)
